...
2. **依赖面**：哪些调用方会受影响
3. **构建面**：哪些构建配置需要更新
4. **每个预期任务的 context_files**：直接修改的文件 + 上游（谁调用/谁传入数据） + 下游（谁消费产出）
5. **变更影响概览**的结构化记录
...
- `depends_on`：前置任务，填 `[]` 或 `[Task N]`。
- `review_profile`：`lightweight` / `standard` / `strict`。
- `context_files`：直接修改文件 + 上游 + 下游，尽量精确到函数 / 组件。
- `verification`：可运行的命令、截图 / DOM 验证、人工检查项。
- `artifacts`：本任务产出的代码、测试、文档、截图、报告。

#### 原则 8：为每个任务标注 review 档位

每个任务必须标注 `review_profile`，供执行阶段选择 reviewer 集：
...
将任务清单写入 `tasks.md`（与 spec.md 同目录）。**严格按以下模板生成，不要遗漏任何章节**：
...
写入 `tasks.md` 后，对照以下 checklist 自检（如果自检不通过，修正后重新写入）：
...
9. **review 档位必须标注**：每个任务必须写 `review_profile`，作为执行期分级 review 的依据
