...
| 信号 | 路径 | 执行步骤 |
|------|------|---------|
| 由 `workflow-code-generation` 的 owner/implementer 在 task 内**内嵌调用**（无人值守） | **内嵌路径** | Step 2 → 3 → 4；**跳过 Step 3 用户确认、跳过 Step 5**（测试通过后交还 owner 自跑 review） |
...
创建测试任务清单，**与用户确认**后再继续（**内嵌路径无人值守，跳过确认、直接继续**）：
...
> 仅**完整流程**执行本步骤。**内嵌路径不执行 Step 5**——测试通过后直接交还 owner 子 agent（它会自跑 code-review），不提示用户手动说 CR。
>
> **前置条件**：所有测试用例已通过。
...
1. **测试计划必须经用户确认后才能生成代码**（例外：被 `workflow-code-generation` 内嵌调用的无人值守路径免确认，直接生成）
