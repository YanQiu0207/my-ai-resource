...
1. 加载编码规范（同步骤 4）。
2. 实现改动（动代码前若有 `verify.config.json`，先 `workflow-verification` 采基线）。发现需改多个文件 → **立即退出**，转标准流程。
3. **自动运行 code review**：加载 `workflow-code-review`（`review_profile: lightweight`），有 finding → 自行修复后重跑，直到通过。
4. **机器验证**：加载 `workflow-verification`（有 config 比基线、否则探测 build / test），绿才继续；失败回第 2 步修复。
...
3. **机器验证**：对合并结果整体跑 `workflow-verification`（有 config 按配置比基线、否则探测 build / test）；FAIL → 派 fix agent 修复后回第 2 步重审再验，仍 FAIL 标 `需人工`；未验到列为风险。
