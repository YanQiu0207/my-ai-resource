...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_01.gif)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_02.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_03.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_04.png)

#### 3.1 整体目录结构

```
llm-knowledge/├── backend/│   ├── overview.md                  # 全局总览│   ├── business/│   │   ├── private_domain/          # 商城会员域│   │   ├── activity/                # 活动平台域│   │   ├── yybgame/                 # 福利平台域│   │   │   ├── meta.yaml            # 服务索引│   │   │   ├── custom/              # 域级手工文档│   │   │   └── service_name/│   │   │       ├── overview.md      # 等8类自动生成│   │   │       └── custom/          # 服务级手工文档│   │   └── ...                      # 更多业务域│   └── common/                      # 公共知识库│       ├── conventions/             # 开发规范│       ├── lib_usage/               # 常用库指南│       └── tech/                    # 技术专题└── .codebuddy/skills/
```
...
| 位置 | 用途 |
| --- | --- |
| `{service}/custom/`<br>或 `{service}/custom.md` | 服务级手工文档：业务背景、调用示例、架构决策、历史包袱说明等 AI 难以从代码推断的知识 |
| `{group}/custom/`<br>或 `{group}/custom.md` | 业务域级手工文档：跨服务的业务流程说明、领域概念定义、常见问题汇总 |
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_05.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_06.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_07.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_08.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_09.png)
...
|  | `interface-verifier` | `trpc-gateway`<br>调接口 + 断言 + galileo 根因分析（只诊断不改） |
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_10.png)
...
```
Phase R: 产品需求拆解  ↓Fork 段（并行）：每个子需求各自跑 Phase 1~4  ↓Join 段（串行收口）：合并 → 发布 → 验证 → 提交
```
...
| `Shared file`<br>(如 `main.go`等) | 不在并行执行中修改，统一收敛到集成收口阶段由单个 Agent 执行，避免多 Agent 修改同一入口 |
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_11.png)
...
在 AI 生产代码效率越来越高的当下，有一种说法是“忘记代码的存在”：人不再逐行审查 AI 写的代码，只关心运行结果是否符合预期。YouTube 上有一个来自 Anthropic 研究员的演讲，分享的就是这种观点：[Vibe coding in prod](https://www.youtube.com/watch?v=fHWFF_pnqDk)[获取直链](http://savefrom.net/?url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3DfHWFF_pnqDk&utm_source=userjs-chrome&utm_medium=extensions&utm_campaign=link_modifier "获取直链")
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_12.png)

![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_13.png)
...
![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_14.gif)

![图片](assets/从AI_Coding到Harness_Engineering的端到端工程开发实践_15.png)